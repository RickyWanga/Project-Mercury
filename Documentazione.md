...
Per questo si ha scelto di abbandonare la matrice e usare due code come struttura per la gestione degli Hittable, in quanto questi compaiono dall'alto e scompaiono dal basso tramite una logica FIFO. In questo modo l'operazione moveDown() è passata da avere costo quadratico a costo lineare, in quanto esso è proporzionale soltanto al numero di Hittable presenti sul campo, che a sua volta è proporzionale soltanto all'altezza del campo (O(n)).
